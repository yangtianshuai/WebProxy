...
namespace Web_Proxy
{
    ...
    {
        public static INetFwOpenPort GetRule(string name, string protocol)
        {
            var netFwMgr = (INetFwMgr)Activator.CreateInstance(Type.GetTypeFromProgID("HNetCfg.FwMgr"));
            NET_FW_IP_PROTOCOL_ _protocal;
            if (protocol.ToUpper() == "TCP")
            {
                _protocal = NET_FW_IP_PROTOCOL_.NET_FW_IP_PROTOCOL_TCP;
            }
            else
            {
                _protocal = NET_FW_IP_PROTOCOL_.NET_FW_IP_PROTOCOL_UDP;
            }

            foreach (INetFwOpenPort mPort in netFwMgr.LocalPolicy.CurrentProfile.GloballyOpenPorts)
            {
                if (mPort.Name == name && mPort.Protocol == _protocal)
                {
                    return mPort;
                }
            }
        ...
        public static void AddRule(string name, string executablePath)
        {
            //创建firewall管理类的实例
        ...
        /// <param name="port">端口</param>
        /// <param name="protocol">协议（TCP、UDP）</param>
        public static void DeleteRule(int port, string protocol)
        {
            INetFwMgr netFwMgr = (INetFwMgr)Activator.CreateInstance(Type.GetTypeFromProgID("HNetCfg.FwMgr"));
            if (protocol == "TCP")
            {
        ...
        public static void DeleteRule(string executablePath)
        {
            INetFwMgr netFwMgr = (INetFwMgr)Activator.CreateInstance(Type.GetTypeFromProgID("HNetCfg.FwMgr"));
